"""Neural Network Module — layers, activations, containers (pybind11 backend)."""
...
class LayerNorm(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return Tensor.from_numpy(out, dtype=x.dtype_name)
...
class RMSNorm(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return Tensor.from_numpy(arr / rms * self.weight.data, dtype=x.dtype_name)
...
class MultiheadAttention(Module):
    ...
    def forward(self, q: Tensor, k: Tensor = None, v: Tensor = None, mask: Tensor = None) -> Tensor:
        ...
        H = self.num_heads
        D = self.head_dim
        q_np = self.q_proj(q).data.reshape(B, T, H, D).transpose(0, 2, 1, 3)
        k_np = self.k_proj(k).data.reshape(B, -1, H, D).transpose(0, 2, 1, 3)
        v_np = self.v_proj(v).data.reshape(B, -1, H, D).transpose(0, 2, 1, 3)
        ...
        return self.o_proj(Tensor.from_numpy(out_np, dtype=q.dtype_name))
