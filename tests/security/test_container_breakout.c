...
int main() {
    ...
    int r = arix_breakout_detect_ns_change(1234, "malicious");
    printf("NS change detect: %d\n", r);
    r = arix_breakout_detect_mount(1234, "/etc/passwd", "malicious");
    printf("Mount detect (suspicious path): %d\n", r);
    r = arix_breakout_detect_capability(1234, 0xFFFFFFFF, "malicious");
    printf("Capability detect: %d\n", r);
    r = arix_breakout_detect_syscall(1234, "mount", "malicious");
    printf("Syscall detect: %d\n", r);
    r = arix_breakout_check_file_access("/etc/shadow");
    printf("File access detect: %d\n", r);
    arix_breakout_add_watch_file("/var/run/docker.sock");
    arix_breakout_get_stats(&stats);
