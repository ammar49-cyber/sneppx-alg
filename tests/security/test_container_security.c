...
int main() {
    ...
    if (arix_container_parse_manifest(test_manifest, sizeof(test_manifest), &manifest) != 0) {
        printf("FAIL: parse manifest\n"); return 1;
    }
    printf("Manifest parsed: mediaType=%s layers=%d\n", manifest.media_type, manifest.num_layers);
    sbom_doc_t doc;
    arix_sbom_generate(&doc, "arix-algo:latest", "0.7.5");
    arix_sbom_add_component(&doc, "libssl", "3.0.12", "library", "OpenSSL Foundation");
    arix_sbom_add_component(&doc, "libcurl", "8.4.0", "library", "curl project");
    if (arix_sbom_validate(&doc) != 0) { printf("FAIL: SBOM validate\n"); return 1; }
    char json[4096];
    arix_sbom_export_json(&doc, json, sizeof(json));
    printf("SBOM JSON:\n%s\n", json);
    arix_container_init_vuln_db();
    vuln_result_t results[10];
    int n = arix_container_scan_vulns("nginx:latest", NULL, 0, results, 10);
    printf("Found %d vulnerabilities for nginx\n", n);
    container_stats_t stats;
    arix_container_get_stats(&stats);
